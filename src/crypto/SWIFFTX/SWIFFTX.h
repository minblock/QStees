...
void ComputeQtipArraygleSWIFFT(unsigned char *input, unsigned short m,
					  	 unsigned char output[SWIFFTX_OUTPUT_BLOCK_SIZE],
						 const swift_int16_t *a);

// Computes the result of a single SWIFFTX operation.
// NOTE: for simplicity we use 'ComputeQtipArraygleSWIFFT()' as a subroutine. This is only to show
